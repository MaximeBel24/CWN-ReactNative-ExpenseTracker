...
export const deleteTransaction = async (
  ...
  try {
    ...
    // check fields to be updated based transaction type
    const updateType =
...
export const fetchWeeklyStats = async (uid: string): Promise<ResponseType> => {
  try {
    ...
    const transacrtionsQuery = query(
    ...
    const querySnapshot = await getDocs(transacrtionsQuery);
...
export const fetchMonthlyStats = async (uid: string): Promise<ResponseType> => {
  try {
    ...
    return {
      ...
      data: {
        stats,
        transactions, // Include all transactions details
      },
    };
  } catch (err: any) {
    console.log("error fetching weekly stats", err);
